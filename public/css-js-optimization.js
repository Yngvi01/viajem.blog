...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const optimizeCSS = () => {
    ...
    if (
      unusedSelectors.size > 0 &&
      window.localStorage.getItem("DEBUG_CSS") === "true"
    ) {
      console.log(
        "Potenciais seletores CSS não utilizados:",
  ...
  const optimizeJavaScript = () => {
    ...
    document
      .querySelectorAll('script:not([async]):not([defer]):not([type="module"])')
      .forEach((script) => {
